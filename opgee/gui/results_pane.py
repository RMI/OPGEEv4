...
_zero_flow_at_boundary_msg = {
    "layout": {
        ...
        "yaxis": {"visible": False},
        "annotations": [
            {"text": "No figure", "xref": "paper", "yref": "paper", "showarrow": False, "font": {"size": 20}}
        ],
    }
}
...
class ResultsPane(OpgeePane):
    ...
    def add_callbacks(self):
        ...
        @app.callback(
            Output("ci-text", "children"), Input("run-button", "n_clicks"), Input("analysis-and-field", "data")
        )
        ...
        def ci_barchart(ci_text, analysis_and_field):
            ...
            fig = go.Figure(
                data=[
                    go.Bar(name=row.category, x=[row.unit], y=[row.value], width=[0.7]) for idx, row in df.iterrows()
                ],
                layout=go.Layout(barmode="stack"),
            )

            fig.update_layout(
                yaxis_title=f"g CO2 per MJ of {fn_unit}",  # doesn't render in latex: r'g CO$_2$ MJ$^{-1}$',
        ...
        def energy_barchart(n_clicks, ci_figure, analysis_and_field):
            ...
            fig = go.Figure(
                data=[
                    go.Bar(name=row.category, x=[row.unit], y=[row.value.m], width=[0.7]) for idx, row in df.iterrows()
                ],
                layout=go.Layout(barmode="stack"),
            )

            fig.update_layout(yaxis_title=f"MJ per MJ of {fn_unit}", xaxis_title="Energy consumption")
