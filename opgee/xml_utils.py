        ...
        if p.exists():
            if backup:
                ...
                raise OpgeeException(
                    f"save_xml: file exists: '{path}'; to overwrite specify backup=True or overwrite=True"
                )
...
def attr_to_xml(fields, dtypes, xml_path, analysis_name, modifies="default"):
    ...
    for field_name, col in fields.items():
        field = ET.SubElement(model, "Field", attrib={"name": field_name, "modifies": modifies})
        ...
        for attr, value in col.items():
            ...
            if count == 2:
                ...
                if not (attr_parent := proc_dict.get(process_name)):
                    ...
                    proc_dict[process_name] = attr_parent = ET.SubElement(
                        proc_parent, "Process", attrib={"class": process_name}
                    )
