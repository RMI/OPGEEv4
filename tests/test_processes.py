...
@pytest.mark.parametrize("name, value", [("foo", foo), ("bar", bar), ("baz", baz)])
...
def test_AcidGasRemoval_Aspen(test_model):
    ...
    processing_unit_loss_rate_df = pd.DataFrame(
        data=[0.00041373], index=["AcidGasRemoval"], columns=["loss_rate"], dtype="pint[frac]"
    )
...
def test_AcidGasRemoval_testbook(test_model):
    ...
    processing_unit_loss_rate_df = pd.DataFrame(
        data=[0.000259240], index=["AcidGasRemoval"], columns=["loss_rate"], dtype="pint[frac]"
    )
    field.save_process_data(processing_unit_loss_rate_df=processing_unit_loss_rate_df)
    field.run(analysis)
    proc = field.find_process("AcidGasRemoval")
...
def test_GasDehydration(test_model):
    ...
    processing_unit_loss_rate_df = pd.DataFrame(
        data=[0.00037981], index=["GasDehydration"], columns=["loss_rate"], dtype="pint[frac]"
    )
    field.save_process_data(processing_unit_loss_rate_df=processing_unit_loss_rate_df)
    field.run(analysis)
...
def test_Demethanizer(test_model):
    ...
    processing_unit_loss_rate_df = pd.DataFrame(
        data=[0.0], index=["Demethanizer"], columns=["loss_rate"], dtype="pint[frac]"
    )
    field.save_process_data(processing_unit_loss_rate_df=processing_unit_loss_rate_df)
    field.run(analysis)
...
def test_RyanHolmes(test_model):
    ...
    processing_unit_loss_rate_df = pd.DataFrame(
        data=[0], index=["RyanHolmes"], columns=["loss_rate"], dtype="pint[frac]"
    )
    field.save_process_data(processing_unit_loss_rate_df=processing_unit_loss_rate_df)
    field.run(analysis)
