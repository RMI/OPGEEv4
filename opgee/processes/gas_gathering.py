...
class GasGathering(Process):
    def __init__(self, name, **kwargs):
        ...
        self._required_outputs = [("gas for gas dehydration", "gas for gas partition")]
    ...
    def run(self, analysis):
        ...
        loss_rate = self.calculate_site_fugitive_loss_rate(
            input_stream_STP, self.gathering_site_average_site_throughput
        )
    ...
    def calculate_site_fugitive_loss_rate(self, input_stream, site_throughput):
        ...
        loss_rate = ureg.Quantity(
            10
            ** (self.site_fugitive_intercept.to("frac").m + self.site_fugitive_slope.to("frac").m * mass_rate_per_site),
            "frac",
        )
