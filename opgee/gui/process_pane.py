...
class ProcessPane(OpgeePane):
    def get_layout(self, field, show_stream_contents=False, show_disabled_procs=False, layout_name="breadthfirst"):
        # the main row
        layout = html.Div(
            [
                html.Center(
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                **Emissions and energy use**
                                ...
                                html.Pre(id="emissions-and-energy", style={"margin-left": "8px"}),
                            ],
                            style={
                                # 'height': '400px',
    ...
    def add_callbacks(self):
        ...
        def display_edge_data(data, analysis_and_field):
            ...
            if data:
                ...
                with pd.option_context("display.max_rows", None, "display.precision", 3):
                    ...
                    components = (
                        str(nonzero.astype(float)) if nonzero is not None and len(nonzero) else "<empty stream>"
                    )
        ...
        def update_result_table(tab, status, analysis_and_field):
            ...
            def add_children(container, elt):
                ...
                if container.procs:
                    ...
                    div = html.Div(style=style, children=[emissions_table(analysis, sorted_procs)])
        ...
        def redraw_network_graph(show_stream_contents, show_disabled_procs, layout_name, analysis_and_field):
            analysis, field = get_analysis_and_field(model, analysis_and_field)
            return field_network_graph(
                field,
                show_stream_contents=show_stream_contents,
                show_disabled_procs=show_disabled_procs,
                layout_name=layout_name,
...
def field_network_graph(field, show_stream_contents=False, show_disabled_procs=False, layout_name="breadthfirst"):
    ...
    nodes = [
        {"data": {"id": name, "label": name}, "classes": node_class(proc)}
    ...
    edges = [
        {
            "data": {"id": name, "source": s.src_name, "target": s.dst_name, "contents": ", ".join(s.contents)},
            "classes": edge_class(s),
        }
        for name, s in field.stream_dict.items()
        if (s.dst_proc and s.src_proc and (show_disabled_procs or (s.dst_proc.enabled and s.src_proc.enabled)))
    ]
    ...
    layout = cyto.Cytoscape(
        ...
        layout={"name": layout_name, "roots": '[id = "Reservoir"]'},
        stylesheet=[
            {
                "selector": "node",
                "style": {
