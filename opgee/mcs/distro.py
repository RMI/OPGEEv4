...
class Empirical:
    ...
    @classmethod
    def from_csv(
        cls, pathname=None, colname=None
    ):  # both are required args, but must be keywords to use with DistroGen.makeRV()
        import pandas as pd
        ...
        if df is None:
            try:
                csvdata = (
                    pathname if os.path.isabs(pathname) else resourceStream(pathname, stream_type="bytes", decode=None)
                )
