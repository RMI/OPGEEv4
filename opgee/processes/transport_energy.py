...
class TransportEnergy(OpgeeObject):
    ...
    def get_transport_energy_dict(
        self, field, parameter_table, transport_share_fuel, transport_by_mode, LHV_rate, prod_type
    ):
        """
        ...
        ocean_tanker_orig_dest_energy_intensity = self.transport_energy_intensity(
        ...
        ocean_tanker_dest_orig_energy_intensity = self.transport_energy_intensity(
        ...
        barge_orig_dest_energy_intensity = self.transport_energy_intensity(
        ...
        barge_dest_orig_energy_intensity = self.transport_energy_intensity(
        ...
        pipeline_orig_dest_energy_intensity = (
            energy_intensity_pipeline_turbine * frac_power_pipeline_turbine
            + energy_intensity_pipeline_engine_current * frac_power_pipeline_engine_current
            + energy_intensity_pipeline_engine_future * frac_power_pipeline_engine_future
        )

        transport_orig_dest_energy_consumption = pd.Series(
            [
                ocean_tanker_orig_dest_energy_intensity,
                barge_orig_dest_energy_intensity,
                pipeline_orig_dest_energy_intensity,
                energy_intensity_rail_transport,
                self.energy_intensity_truck,
            ],
            dtype="pint[btu/tonne/mile]",
        )

        transport_dest_origin_energy_consumption = pd.Series(
            [
                ocean_tanker_dest_orig_energy_intensity,
                barge_dest_orig_energy_intensity,
                self.pipeline_dest_orig_energy_intensity,
                self.rail_dest_orig_energy_intensity,
                self.truck_dest_orig_energy_intensity,
            ],
            dtype="pint[btu/tonne/mile]",
        )
        ...
        transport_energy_consumption = (
            transport_orig_dest_energy_consumption + transport_dest_origin_energy_consumption
        ) / denominator

        fuel_consumption = TransportEnergy.fuel_consumption(
            fraction_transport,
            transport_distance,
    ...
    def get_parameter_dict(parameter_table):
        ...
        parameter_dict = {
            name: ureg.Quantity(float(value), parameter_unit[name]) for name, value in parameter_value.items()
        }

        return parameter_dict

    def transport_energy_intensity(
        self, load_factor, type, ocean_tanker_speed=None, ocean_tanker_size=None, barge_capacity=None, barge_speed=None
    ):
        """
        ...
        hp = (
            ureg.Quantity(9070 + 0.101 * ocean_tanker_size.m, "hp")
            if type == "tanker"
    ...
    def fuel_consumption(
        fraction_transport, transport_distance, transport_share_fuel, transport_energy_consumption, feed_loss, LHV
    ):
        """
